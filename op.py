...
class stock_fn:
    ...
    def sell_fn(self, price, count):
        """ 주식을 매도하는 함수 """
        if (self.stock < count):
            error_msg = '그 정도 주식을 가지고 있지 않습니다.'
    ...
    def print_profit(self):
        print("{0:->30}".format(""))
        a = self.cash - self.first
        print(f"순이익 : {self.cash - self.first}")
        b = (self.cash / self.first) * 100 - 100
...
def if_used(data, person, buy_value, sell_value):
    ...
    buy_condition = (rsi <= buy_value["rsi_val"]) & (mfi <= buy_value["mfi_val"]) & (macd - macd_sig < buy_value["macd_val"])  # 매수하는 조건
    sell_condition = (rsi >= sell_value["rsi_val"]) & (mfi >= sell_value["mfi_val"]) & (macd - macd_sig >= sell_value["macd_val"])  # 매도하는 조건
